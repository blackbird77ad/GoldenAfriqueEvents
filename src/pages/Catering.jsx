      ...
      <div className="relative py-20 px-4 overflow-hidden bg-black text-white text-center">
        ...
        <div className="relative z-10">
          ...
          <p className="text-gray-300 max-w-xl mx-auto text-base">
            Select the dishes you want and send your order directly via WhatsApp, phone or Instagram DM.
          </p>
        </div>
      </div>
      ...
      <div className="bg-yellow-50 border-b border-yellow-100 py-6 px-4">
        <div className="max-w-3xl mx-auto">
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 text-sm text-yellow-900">
            ...
            <div className="flex items-center gap-2"><span className="w-6 h-6 rounded-full bg-yellow-400 text-black font-extrabold text-xs flex items-center justify-center">2</span> Tap the cart to review</div>
            <div className="hidden sm:block text-yellow-300">→</div>
            <div className="flex items-center gap-2"><span className="w-6 h-6 rounded-full bg-yellow-400 text-black font-extrabold text-xs flex items-center justify-center">3</span> Send order via WhatsApp or Instagram DM</div>
          </div>
        </div>
      </div>
              ...
              return (
                <motion.div key={item.id} layout
                  ...
                  <div className="relative aspect-square overflow-hidden bg-gray-50">
                    ...
                    {/* ID badge */}
                    <span className="absolute top-2 left-2 bg-black/70 text-yellow-400 text-xs font-extrabold px-2 py-0.5 rounded-full">
                      {item.id}
                    </span>
                  </div>
      ...
      <AnimatePresence>
        {cateringCount > 0 && (
          <motion.button
            ...
            <ShoppingBag size={20} />
            <span>{cateringCount} item{cateringCount !== 1 ? 's' : ''} selected</span>
            ...
            <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }}
                  ...
                  <div key={item.id} className="flex items-center gap-3 p-3 rounded-2xl bg-gray-50">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1.5">
                        <button onClick={() => updateQty(item.id, item.qty - 1, 'catering')}
                    ...
                    <button onClick={() => removeFromCart(item.id, 'catering')} className="text-gray-300 hover:text-red-500 transition-colors">
                      <Trash2 size={16} />
                    </button>
                  </div>
                ))}
              ...
              <div className="p-5 border-t border-gray-100 bg-gray-50">
                <p className="text-xs text-gray-500 mb-4 text-center leading-relaxed">
                  Send your selection to us — we'll confirm availability and pricing, then finalise your order.
                </p>
