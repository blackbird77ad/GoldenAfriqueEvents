  ...
  return (
    <div className="min-h-screen bg-white pt-16 md:pt-20">
      ...
      <div className="bg-yellow-50 border-b border-yellow-100 py-5 px-4">
        <div className="max-w-4xl mx-auto flex flex-col sm:flex-row items-start sm:items-center gap-4 text-sm text-yellow-900">
          <div className="flex items-center gap-2"><span className="w-6 h-6 rounded-full bg-yellow-400 text-black font-extrabold text-xs flex items-center justify-center">1</span> Browse & note the item ID (e.g. R001)</div>
          ...
          <div className="hidden sm:block text-yellow-300">→</div>
          <div className="flex items-center gap-2"><span className="w-6 h-6 rounded-full bg-yellow-400 text-black font-extrabold text-xs flex items-center justify-center">3</span> Send enquiry to our events team</div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-10">
        ...
        <input value={searchTerm} onChange={e => setSearchTerm(e.target.value)}
          placeholder="Search by name or ID (e.g. R001, chairs, backdrop)..."
          ...
          <>
            ...
            <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }}
              ...
              <div className="flex justify-between items-center p-5 border-b border-gray-100">
                ...
                <button onClick={() => setCartOpen(false)} className="w-9 h-9 rounded-full border-2 border-gray-100 flex items-center justify-center hover:border-black transition-all"><X size={16} /></button>
              </div>
              <div className="flex-1 overflow-y-auto p-5 flex flex-col gap-3">
                {rentalCart.map(item => (
                  <div key={item.id} className="flex items-center gap-3 p-3 rounded-2xl bg-gray-50">
                    <img src={item.image} alt={item.name} className="w-14 h-14 rounded-xl object-cover shrink-0 bg-gray-100" onError={e => { e.target.style.display = 'none'; }} />
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1.5">
                        <button onClick={() => updateQty(item.id, item.qty - 1, 'rental')} className="w-6 h-6 rounded-full border border-gray-300 flex items-center justify-center hover:border-black transition-all"><Minus size={10} /></button>
                        <span className="font-extrabold text-sm">{item.qty}</span>
                        <button onClick={() => updateQty(item.id, item.qty + 1, 'rental')} className="w-6 h-6 rounded-full border border-gray-300 flex items-center justify-center hover:border-black transition-all"><Plus size={10} /></button>
                      </div>
                    </div>
                    <button onClick={() => removeFromCart(item.id, 'rental')} className="text-gray-300 hover:text-red-500 transition-colors"><Trash2 size={16} /></button>
                  </div>
                ))}
              </div>
              <div className="p-5 border-t border-gray-100 bg-gray-50">
                <p className="text-xs text-gray-500 mb-4 text-center leading-relaxed">Our events team will confirm availability, pricing, delivery and setup.</p>
                <div className="flex flex-col gap-2">
                  <button onClick={sendInq} className="w-full py-3.5 rounded-2xl bg-green-500 text-white font-extrabold text-sm flex items-center justify-center gap-2 hover:bg-green-600 transition-all">
                    <MessageCircle size={18} /> Send to Events Team (WhatsApp)
                  </button>
