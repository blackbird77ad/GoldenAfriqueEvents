...
const S = {
  ...
  badge:  (ok) => ({ fontSize: '0.62rem', fontWeight: 800, padding: '3px 8px', borderRadius: 20, background: ok ? '#d1fae5' : '#fee2e2', color: ok ? '#065f46' : '#991b1b' }),
  btn:    (primary) => ({ display:'flex', alignItems:'center', gap:6, padding:'9px 18px', borderRadius:8, fontWeight:900, fontSize:'0.78rem', letterSpacing:'0.06em', textTransform:'uppercase', border:'none', cursor:'pointer', background: primary ? 'var(--brown-dark)' : '#fff', color: primary ? 'var(--gold)' : 'var(--brown-dark)', border: primary ? 'none' : '1.5px solid var(--brown-pale)', transition:'all 0.18s' }),
  input:  { width:'100%', border:'1.5px solid var(--brown-pale)', borderRadius:8, padding:'10px 14px', fontSize:'0.84rem', outline:'none', fontFamily:'inherit', background:'#fff', color:'var(--brown-dark)' },
};
...
function ImageEditor({ src: imgSrc, onApply, onCancel }) {
  ...
  useEffect(() => {
    ...
    img.onload  = () => { setImgLoaded(true); setError(false); };
  ...
  const draw = useCallback(() => {
    ...
    ctx.lineWidth   = 1;
    const t = pad, b = CANVAS_H - pad, l = pad, r = CANVAS_W - pad;
  ...
  const reset = () => { setScale(1); setOffset({ x:0, y:0 }); setRotate(0); };
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
      {error ? (
        ...
        <>
          {/* Canvas */}
          <div style={{ position:'relative', borderRadius:10, overflow:'hidden', border:'2px solid var(--gold)', cursor: dragging ? 'grabbing' : 'grab', userSelect:'none', background:'#111' }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:10 }}>
            ...
            <span style={{ fontSize:'0.72rem', fontWeight:800, color:'var(--brown)', minWidth:36 }}>{Math.round(scale * 100)}%</span>
          </div>
...
function ItemModal({ item, type, onSave, onClose }) {
  ...
  const [imgMode,   setImgMode]   = useState('url');  // 'url' | 'editor' | 'preview'
  ...
  /* Compress to max 800px wide, JPEG 0.82 quality before opening editor */
  const compressAndEdit = (file) => {
    ...
    reader.onload = ev => {
      ...
      img.onload = () => {
        ...
        setImgMode('editor');
        setSizeInfo(`Original: ${(file.size / 1024).toFixed(0)} KB → Compressed: ${(compressed.length * 0.75 / 1024).toFixed(0)} KB`);
      };
  ...
  const handleFileUpload = e => { compressAndEdit(e.target.files[0]); };
  ...
  return (
    <div style={{ position:'fixed', inset:0, zIndex:100, display:'flex', alignItems:'center', justifyContent:'center', padding:'1rem', background:'rgba(44,26,6,0.65)', backdropFilter:'blur(4px)' }}
      onClick={onClose}>
      <motion.div initial={{ y:30, opacity:0 }} animate={{ y:0, opacity:1 }}
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'1.4rem' }}>
          ...
          <button onClick={imgMode === 'editor' ? () => setImgMode('url') : onClose}
        ...
        {/* EDITOR MODE */}
        {imgMode === 'editor' ? (
          <ImageEditor src={editorSrc} onApply={applyEdit} onCancel={() => setImgMode('url')} />
        ) : (
          <>
            {error && <div style={{ background:'#fee2e2', border:'1px solid #fca5a5', color:'#991b1b', fontSize:'0.8rem', padding:'10px 14px', borderRadius:8, marginBottom:16 }}>{error}</div>}

            <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
              <div>
                <label style={{ fontSize:'0.68rem', fontWeight:800, letterSpacing:'0.14em', textTransform:'uppercase', color:'var(--brown)', display:'block', marginBottom:5 }}>Name *</label>
                <input value={form.name} onChange={e => set('name', e.target.value)} style={S.input} placeholder="e.g. Jollof Rice" />
              </div>

              <div>
                <label style={{ fontSize:'0.68rem', fontWeight:800, letterSpacing:'0.14em', textTransform:'uppercase', color:'var(--brown)', display:'block', marginBottom:5 }}>Description</label>
                <textarea value={form.desc} onChange={e => set('desc', e.target.value)} rows={2} style={{ ...S.input, resize:'none' }} placeholder="Short description..." />
              </div>

              <div>
                <label style={{ fontSize:'0.68rem', fontWeight:800, letterSpacing:'0.14em', textTransform:'uppercase', color:'var(--brown)', display:'block', marginBottom:5 }}>Category *</label>
              ...
              {/* IMAGE SECTION */}
              <div>
                <label style={{ fontSize:'0.68rem', fontWeight:800, letterSpacing:'0.14em', textTransform:'uppercase', color:'var(--brown)', display:'block', marginBottom:8 }}>Image</label>

                {/* Tab: URL vs Upload */}
                <div style={{ display:'flex', gap:4, background:'var(--gold-pale)', borderRadius:8, padding:3, marginBottom:10, width:'fit-content' }}>
                  {[['url','Paste URL'],['upload','Upload File']].map(([v,label]) => (
                    <button key={v} onClick={() => setImgMode(v === imgMode ? imgMode : v)}
                      type="button"
                      style={{ padding:'6px 14px', borderRadius:6, border:'none', fontWeight:800, fontSize:'0.72rem', cursor:'pointer', transition:'all 0.2s',
                ...
                {imgMode === 'upload' ? (
                  <div>
                    <div onClick={() => fileRef.current?.click()}
                      onDragOver={e => { e.preventDefault(); e.currentTarget.style.background='var(--gold-cream)'; }}
                      onDragLeave={e => e.currentTarget.style.background='var(--gold-pale)'}
                      onDrop={e => { e.preventDefault(); e.currentTarget.style.background='var(--gold-pale)'; compressAndEdit(e.dataTransfer.files[0]); }}
                      ...
                      onMouseEnter={e => e.currentTarget.style.background='var(--gold-cream)'}
                      onMouseLeave={e => e.currentTarget.style.background='var(--gold-pale)'}>
                      <div style={{ fontSize:'1.8rem', marginBottom:8 }}>📁</div>
                      <div style={{ fontWeight:800, fontSize:'0.82rem', color:'var(--brown-dark)', marginBottom:4 }}>Click or drag image here</div>
                      <div style={{ fontSize:'0.7rem', color:'var(--brown-light)' }}>JPG, PNG, WEBP supported. Auto-compressed to save space.</div>
                    </div>
                    <input ref={fileRef} type="file" accept="image/*" onChange={handleFileUpload} style={{ display:'none' }} />
                    {sizeInfo && <div style={{ marginTop:8, fontSize:'0.68rem', color:'#065f46', fontWeight:800, textAlign:'center', background:'#d1fae5', borderRadius:6, padding:'4px 10px' }}>{sizeInfo}</div>}
                  </div>
                ) : (
                  <>
                    <input value={form.image.startsWith('data:') ? '(Cropped image applied)' : form.image}
                      onChange={e => { set('image', e.target.value); setImgMode('url'); }}
              ...
              <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                <button type="button" onClick={() => set('available', !form.available)}
                  ...
                  <span style={{ position:'absolute', top:3, width:18, height:18, borderRadius:'50%', background:'#fff', boxShadow:'0 1px 4px rgba(0,0,0,0.2)', transition:'left 0.2s', left: form.available ? 23 : 3 }}/>
                </button>
            ...
            <div style={{ display:'flex', gap:10, marginTop:'1.5rem' }}>
              <button onClick={onClose} style={{ flex:1, padding:'12px', borderRadius:8, border:'1.5px solid var(--brown-pale)', background:'none', fontWeight:800, fontSize:'0.82rem', cursor:'pointer', color:'var(--brown)' }}>
                Cancel
              </button>
              <button onClick={submit} disabled={saving} style={{ flex:1, padding:'12px', borderRadius:8, border:'none', background:'var(--brown-dark)', color:'var(--gold)', fontWeight:900, fontSize:'0.82rem', cursor:'pointer', opacity: saving ? 0.6 : 1 }}>
...
function PinLogin({ onLogin, isSetup }) {
  ...
  const submit = async () => {
    ...
    try {
      if (isSetup) await api('/api/auth/setup', { method:'POST', body: JSON.stringify({ pin }) });
    ...
    <div style={{ minHeight:'100vh', background:'var(--brown-dark)', display:'flex', alignItems:'center', justifyContent:'center', padding:'1rem' }}>
      <motion.div initial={{ opacity:0, y:24 }} animate={{ opacity:1, y:0 }}
        ...
        <div style={{ textAlign:'center', marginBottom:'2rem' }}>
          <div style={{ width:56, height:56, background:'var(--gold)', borderRadius:12, display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 1rem', fontSize:'1.5rem' }}>🔐</div>
        ...
        {err && <div style={{ background:'#fee2e2', color:'#991b1b', fontSize:'0.78rem', padding:'10px 14px', borderRadius:8, marginBottom:14, textAlign:'center' }}>{err}</div>}
...
function ItemCard({ item, onEdit, onDelete, onToggle }) {
  return (
    <motion.div layout initial={{ opacity:0, scale:0.96 }} animate={{ opacity:1, scale:1 }}
      ...
      {/* Image */}
      <div style={{ position:'relative', aspectRatio:'1/1', background:'var(--gold-pale)', flexShrink:0 }}
        onClick={() => onEdit(item)}>
        {item.image
        ...
        <div style={{ position:'absolute', top:6, left:6 }}>
          <span style={{ background:'rgba(44,26,6,0.85)', color:'var(--gold)', fontSize:'0.58rem', fontWeight:900, padding:'3px 7px', borderRadius:4 }}>{item.itemId}</span>
        </div>
      ...
      <div style={{ padding:'10px 12px', display:'flex', flexDirection:'column', gap:4, flex:1 }}>
        <div style={{ fontSize:'0.62rem', color:'var(--brown-light)', fontWeight:800, textTransform:'uppercase', letterSpacing:'0.1em' }}>{item.category}</div>
        <div style={{ fontWeight:900, fontSize:'0.84rem', color:'var(--brown-dark)', lineHeight:1.3 }}>{item.name}</div>
        {item.desc && <div style={{ fontSize:'0.72rem', color:'var(--brown)', lineHeight:1.5, display:'-webkit-box', WebkitLineClamp:2, WebkitBoxOrient:'vertical', overflow:'hidden' }}>{item.desc}</div>}
      </div>

      {/* Actions */}
      <div style={{ display:'flex', borderTop:'1px solid var(--brown-pale)', padding:'0' }}>
        <button onClick={() => onToggle(item._id)} title={item.available ? 'Hide from menu' : 'Show on menu'}
...
function ItemRow({ item, onEdit, onDelete, onToggle }) {
  return (
    <motion.div layout initial={{ opacity:0 }} animate={{ opacity:1 }}
      ...
      onMouseLeave={e => e.currentTarget.style.borderColor='var(--brown-pale)'}>
...
export default function Admin() {
  ...
  const [avail,      setAvail]      = useState('All'); // 'All' | 'Live' | 'Off'
  ...
  useEffect(() => {
    if (authed) { load('catering'); load('rentals'); }
  }, [authed]);

  useEffect(() => { setPage(1); }, [search, catFilter, avail, tab]);

  const load = async (type) => {
    setLoading(true);
    try {
      const data = await api(`/api/${type}`);
      type === 'catering' ? setCatering(data) : setRentals(data);
    } catch {}
    finally { setLoading(false); }
  };
  ...
  const handleSave = async (type, form, existing) => {
    if (existing) {
      const u = await api(`/api/${type}/${existing._id}`, { method:'PUT', body:JSON.stringify(form) });
      type === 'catering' ? setCatering(c => c.map(i => i._id === u._id ? u : i)) : setRentals(r => r.map(i => i._id === u._id ? u : i));
    } else {
      const u = await api(`/api/${type}`, { method:'POST', body:JSON.stringify(form) });
      type === 'catering' ? setCatering(c => [u, ...c]) : setRentals(r => [u, ...r]);
    }
  };

  const handleDelete = async (id, type) => {
    await api(`/api/${type}/${id}`, { method:'DELETE' });
    type === 'catering' ? setCatering(c => c.filter(i => i._id !== id)) : setRentals(r => r.filter(i => i._id !== id));
    setDeleteConf(null);
  };
  ...
  if (!authed) return <PinLogin onLogin={() => setAuthed(true)} isSetup={isSetup} />;
  ...
  const filtered = useMemo(() => allItems.filter(item => {
    ...
    const matchSearch = !q || item.name.toLowerCase().includes(q) || (item.itemId || '').toLowerCase().includes(q) || item.category.toLowerCase().includes(q);
    ...
    <div style={{ minHeight:'100vh', background:'var(--cream)', fontFamily:'inherit' }}>
      ...
      <div style={{ background:'var(--brown-dark)', borderBottom:'3px solid var(--gold)', padding:'0 1.5rem', display:'flex', alignItems:'center', justifyContent:'space-between', height:60, position:'sticky', top:0, zIndex:50 }}>
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          ...
          <div>
            ...
            <div style={{ fontSize:'0.6rem', color:'var(--brown-light)', letterSpacing:'0.12em', textTransform:'uppercase' }}>Golden Afrique Event</div>
          </div>
        </div>
        <div style={{ display:'flex', gap:8 }}>
          <button onClick={() => load(tab)} style={{ width:34, height:34, borderRadius:8, border:'1px solid rgba(245,200,66,0.3)', background:'none', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center' }}>
            <RefreshCw size={14} color="var(--gold)" />
          </button>
          <button onClick={logout} style={{ display:'flex', alignItems:'center', gap:6, padding:'7px 14px', borderRadius:8, border:'1px solid rgba(245,200,66,0.3)', background:'none', cursor:'pointer', color:'var(--gold)', fontWeight:800, fontSize:'0.76rem' }}>
      ...
      <div style={{ maxWidth:1400, margin:'0 auto', padding:'1.5rem' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(170px, 1fr))', gap:'1rem', marginBottom:'1.5rem' }}>
          {[
            ...
            <div key={i} style={{ background:m.bg, border:'1.5px solid var(--brown-pale)', borderRadius:10, padding:'1.1rem 1.2rem', display:'flex', gap:12, alignItems:'center' }}>
              <div style={{ width:38, height:38, borderRadius:9, background:'rgba(255,255,255,0.7)', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>{m.icon}</div>
              <div>
                <div style={{ fontSize:'1.5rem', fontWeight:900, color:'var(--brown-dark)', lineHeight:1 }}>{m.val}</div>
                <div style={{ fontSize:'0.68rem', fontWeight:800, color:'var(--brown)', textTransform:'uppercase', letterSpacing:'0.1em', marginTop:2 }}>{m.label}</div>
        ...
        <div style={{ display:'flex', gap:4, background:'#fff', padding:4, borderRadius:10, border:'1.5px solid var(--brown-pale)', marginBottom:'1.2rem', width:'fit-content' }}>
          {[['catering','🍽️ Catering'],['rentals','🎪 Rentals']].map(([t, label]) => (
            <button key={t} onClick={() => { setTab(t); setSearch(''); setCatFilter('All'); setAvail('All'); }}
              style={{ padding:'9px 24px', borderRadius:7, border:'none', fontWeight:900, fontSize:'0.82rem', cursor:'pointer', transition:'all 0.2s',
        ...
        <div style={{ display:'flex', flexWrap:'wrap', gap:10, alignItems:'center', marginBottom:'1.2rem' }}>
          ...
          <div style={{ display:'flex', gap:2, background:'var(--gold-pale)', borderRadius:8, padding:3 }}>
            {[['grid', <LayoutGrid size={15}/>], ['list', <List size={15}/>]].map(([v, icon]) => (
              <button key={v} onClick={() => setView(v)}
                style={{ width:32, height:32, borderRadius:6, border:'none', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center',
        ...
        {totalPages > 1 && (
          <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:8, marginTop:'2rem', paddingBottom:'2rem' }}>
            <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1}
              style={{ width:36, height:36, borderRadius:8, border:'1.5px solid var(--brown-pale)', background:page===1?'var(--gold-pale)':'#fff', cursor:page===1?'default':'pointer', display:'flex', alignItems:'center', justifyContent:'center' }}>
              <ChevronLeft size={16} color="var(--brown)" />
            </button>
            {Array.from({ length:totalPages }, (_,i) => i+1).map(n => (
              <button key={n} onClick={() => setPage(n)}
                style={{ width:36, height:36, borderRadius:8, border:'1.5px solid', fontWeight:900, fontSize:'0.82rem', cursor:'pointer',
                  borderColor: n === page ? 'var(--brown-dark)' : 'var(--brown-pale)',
                  background:  n === page ? 'var(--brown-dark)' : '#fff',
                  color:       n === page ? 'var(--gold)' : 'var(--brown-dark)' }}>
                {n}
              </button>
            ))}
            <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page === totalPages}
              style={{ width:36, height:36, borderRadius:8, border:'1.5px solid var(--brown-pale)', background:page===totalPages?'var(--gold-pale)':'#fff', cursor:page===totalPages?'default':'pointer', display:'flex', alignItems:'center', justifyContent:'center' }}>
      ...
      <AnimatePresence>
        {modal && (
          <ItemModal item={modal.item} type={modal.type}
        ...
        {deleteConf && (
          <div style={{ position:'fixed', inset:0, zIndex:100, display:'flex', alignItems:'center', justifyContent:'center', padding:'1rem', background:'rgba(44,26,6,0.6)' }}>
            <motion.div initial={{ scale:0.9, opacity:0 }} animate={{ scale:1, opacity:1 }}
              ...
              <div style={{ fontWeight:900, fontSize:'1rem', color:'var(--brown-dark)', marginBottom:8 }}>Delete this item?</div>
              <div style={{ fontSize:'0.8rem', color:'var(--brown)', marginBottom:'1.5rem', lineHeight:1.7 }}>This cannot be undone. The item will be permanently removed from the catalogue.</div>
              <div style={{ display:'flex', gap:10 }}>
                <button onClick={() => setDeleteConf(null)} style={{ flex:1, padding:'12px', borderRadius:8, border:'1.5px solid var(--brown-pale)', background:'none', fontWeight:800, fontSize:'0.82rem', cursor:'pointer', color:'var(--brown)' }}>
                  Cancel
                </button>
                <button onClick={() => handleDelete(deleteConf.id, deleteConf.type)} style={{ flex:1, padding:'12px', borderRadius:8, border:'none', background:'#dc2626', color:'#fff', fontWeight:900, fontSize:'0.82rem', cursor:'pointer' }}>
                  Delete
                </button>
              </div>
            </motion.div>
